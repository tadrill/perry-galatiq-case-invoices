"""One command that sets everything up and processes the whole invoice inbox.
    ...
    python scripts/demo.py            # build the database, run all 25 invoices
    python scripts/demo.py --serve    # ...then open the web console
    python scripts/demo.py --live     # use the real Grok API instead of offline replay
...
def main() -> int:
    ...
    console.print(
        f"\n[ok]{OK}[/] console on [field]http://127.0.0.1:{args.port}[/]  "
        f"[muted]ctrl-c to stop[/]\n"
    )
    uvicorn.run(
        "invoice_agents.web.app:app", host="127.0.0.1", port=args.port, log_level="warning"
    )
    return 0
